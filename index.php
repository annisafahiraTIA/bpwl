...
include_once("config.php");
    ...
    $result = mysqli_query($conn,"SELECT Monthname(bulan) as bulan,hariHujan,curahHujan FROM hujan ORDER BY MONTH(bulan) ASC");
    $bulan ="";
    $hariHujan="";
    $curahHujan="";
    while ($res = mysqli_fetch_array($result)) {
        $bulan = $bulan."'".$res['bulan']."',";
        $hariHujan = $hariHujan."".$res['hariHujan'].",";
        $curahHujan = $curahHujan."".$res['curahHujan'].",";
    }
    ...
    var data = {
        ...
        series: [
            {
                name: 'Hari Hujan (hari)',
                data: [$hariHujan]
            },
            {
                name: 'Curah Hujan (mm/dt)',
                data: [$curahHujan]
            }
        ]
    };
...
var options = {
    chart: {
        width: 1160,
        height: 650,
        title: 'Grafik Hari Hujan dan Curah Hujan Tahun 2017 di Kecamatan Ngadirojo ',
    },
    yAxis: {
        title: ''
    },
